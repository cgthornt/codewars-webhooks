...
function userRankUpgraded(user) {
  User.findOrUpdate(user);
}
...
exports.parse = function(event, webhookBody, callback) {
  ...
  if(action != 'rank_upgraded')
    return callback(new Error("We only support the user#rank_upgraded action (not the '" + action + "' action)"));

  userRankUpgraded(webhookBody.user);
